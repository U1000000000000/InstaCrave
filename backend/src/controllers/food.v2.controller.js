...
const getFoodItems = catchAsync(async (req, res) => {
  ...
  // Support partial matching for name filter (sanitized to prevent ReDoS)
  if (filters.name) {
    const safeName = normalizeSearchText(filters.name, { maxLength: 100 });
    filters.name = { $regex: escapeRegex(safeName), $options: 'i' };
  }
