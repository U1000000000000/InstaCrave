 ...
 * /api/v1/auth/logout:
 *   post:
 *     summary: Logout the current authenticated principal (user or food partner)
 *     tags: [Auth]
 *     description: |
 *       Convenience endpoint that logs out whichever principal is authenticated.
 *       Equivalent to calling `/api/v1/auth/user/logout` for users or
 *       `/api/v1/auth/food-partner/logout` for food partners.
 *     responses:
 *       200:
...
router.post('/logout',
    ...
    (req, res, next) => {
        if (req.foodPartner) {
            return authController.logoutFoodPartner(req, res, next);
        }
        return authController.logoutUser(req, res, next);
    }
);
