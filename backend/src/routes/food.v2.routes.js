 ...
 *     security:
 *       - bearerAuth: []
 *     description: Upload food with image/video. File signature validated. Role-based rate limiting applies.
 *     requestBody:
 *       required: true
 *       content:
 *         multipart/form-data:
 ...
 * /api/v2/food:
 *   get:
 *     summary: Get paginated and filtered food items
 *     tags: [FoodV2]
 *     security:
 *       - bearerAuth: []
 *     description: Supports pagination, sorting, and filtering by category/price/name/partner. Results cached for 5 minutes.
 *     parameters:
 *       - in: query
 *         name: page
 *         schema:
 ...
 *     security:
 *       - bearerAuth: []
 *     description: Update one field at a time. Partner must own the food item.
 *     parameters:
 *       - in: path
 *         name: id
 *         required: true
 ...
 *   get:
 *     summary: Get food items from followed partners
 *     tags: [FoodV2]
 *     security:
 *       - bearerAuth: []
 *     description: Returns food from partners the user follows. Supports pagination and filtering.
 *     parameters:
 *       - in: query
 *         name: page
 *         schema:
 ...
 *     security:
 *       - bearerAuth: []
 *     description: Toggle like status. Updates like count on food model.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *     security:
 *       - bearerAuth: []
 *     description: Toggle save status. User can retrieve saved items later.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 * /api/v2/food/save:
 *   get:
 *     summary: Get user's saved food items
 *     tags: [FoodV2]
 *     security:
 *       - bearerAuth: []
 *     description: Returns all food items the user has saved. Cached for 5 minutes.
 *     responses:
 *       200:
...
router.get('/save',
  ...
  cacheMiddleware(300, req => {
    ...
    return `user:${req.user._id}:food:v2:saved`;
  }),
 ...
 *     security:
 *       - bearerAuth: []
 *     description: Add a comment to a food item. Comment is sanitized before storage.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *     security:
 *       - bearerAuth: []
 *     description: Fetch all comments on a specific food item. Cached for 3 minutes.
 *     parameters:
 *       - in: query
 *         name: foodId
 *         schema:
 ...
 *   get:
 *     summary: Get comments for a food item (partner)
 *     tags: [FoodV2]
 *     security:
 *       - bearerAuth: []
 *     description: Partner endpoint to view comments on their food items. Cached for 3 minutes.
 *     parameters:
 *       - in: query
 *         name: foodId
 *         schema:
 ...
 *     security:
 *       - bearerAuth: []
 *     description: Partner deletes their own food item. Cascades to delete comments, likes, and saves.
 *     parameters:
 *       - in: path
 *         name: foodId
 *         required: true
 ...
 *     security:
 *       - bearerAuth: []
 *     description: Track when a food item is shared. Increments share counter.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
